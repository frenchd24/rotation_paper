...
v8_err: Test the effect of including +/- rotation error in the models (10/22/18)
...
def model(galaxyName, agnName, fit_NFW = True, include_errs = '+'):
...
#     galaxyName = 'UGC08146'
    ...
    if fit_NFW == True:
        ...
        saveDirectory += '/2x3Rvir/'
    
    if include_errs == '+':
        saveDirectory += '/plusErrs/'
    if include_errs == '-':
        saveDirectory += '/minusErrs/'
        
        
    # make the directory if it does not already exist
    
#     print 'os.path.isdir(saveDirectory) : ',os.path.isdir(saveDirectory)
#     print
...
#         os.mkdir(os.realpathsaveDirectory)
#         
#     print 'os.path.isdir(saveDirectory) : ',os.path.isdir(saveDirectory)
#     print 
#     print 'should have made a directory!'
#     _ = raw_input('Press enter to continue')
#     print
    
    
    print 'starting mkdir...'
    print 'os.path.isdir(saveDirectory) : ',os.path.isdir(saveDirectory)

    if not os.path.exists(os.path.dirname(saveDirectory)):
        try:
            os.makedirs(os.path.dirname(saveDirectory))
...
#         x_lim = 500
        ...
        def fit(x):
            ...
            if x >= 0:
                ...
                if include_errs:
                    if include_errs == '+':
                        y_val = y_val + right_vrot_incCorrected_avg_err
                    if include_errs == '-':
                        y_val = y_val - right_vrot_incCorrected_avg_err
                ...
                if include_errs:
                    if include_errs == '+':
                        y_val = -(y + left_vrot_incCorrected_avg_err)
                    if include_errs == '-':
                        y_val = -(y - left_vrot_incCorrected_avg_err)
                else:
                    y_val = -y
...
#         splineKind = 'cubic'
        ...
        interp_fit = interp1d(xvals, vels, kind=splineKind)
...
#         fig = plt.figure(figsize=(7,5))
        ...
        plot(x_fit, interp_fit(x_fit), color='green', alpha=0.7, ms=0, lw=2, label = r'$\rm Linear-Spline ~Fit$')
        legend()
        ...
        def fit(x):
            ...
            if include_errs:
                if val <0:
                    if include_errs == '+':
                        val = val - right_vrot_incCorrected_avg_err
                    if include_errs == '-':
                        val = val + right_vrot_incCorrected_avg_err
                else:
                    if include_errs == '+':
...
#             agnName = 'SDSSJ112632.90+120437.0'
        ...
        for include_err in include_err_list:
            for fit_NFW in fit_NFW_list:
                print 'galaxyName, agnName: ',galaxyName, agnName
                model(galaxyName, agnName, fit_NFW=fit_NFW, include_errs=include_err)
