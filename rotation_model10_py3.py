...
def steidel_model(vc, inclination, hv, impact, azimuth, y0, Dlos):
    """
...
#         yData = np.array(yData)*-1
        ...
        v200_upperbound = abs(right_vrot_incCorrected_avg)*2.0
        c_lowerbound = 1
        c_upperbound = 20
...
#   midplane_model_projection = project_model_velocity(z, origin, normal, rayDirection, rayPoint, rcutoff, fit, sphericalHalo=False)
    ...
    # unpack the results
    interect            = midplane_model_projection["intersect"]
    intersect_dist      = midplane_model_projection["intersect_dist"]
    dist_from_origin    = midplane_model_projection["dist_from_origin"]
    v_intersect         = midplane_model_projection["v_intersect"]
    v_n_intersect       = midplane_model_projection["v_n_intersect"]
    v_rotation          = midplane_model_projection["v_rotation"]
    v_proj              = midplane_model_projection["v_proj"]
    
    print 'interect: ',interect
    print 'intersect_dist: ',intersect_dist
...
#     for i in arange(-rcutoff, rcutoff, steidel_step):
    ...
    for i in arange(min(intersect_list), max(intersect_list), steidel_step):
        # --- steidel_model(vmax, inclination, hv, impact, azimuth, y0, Dlos)
        #
...
#         print
        ...
        # --- the model
        vlos = steidel_model(effective_vc, effectiveInc, hv, impact, az, y0, i)
...
#         print
        ...
        x_tick_spacing = round((max(intersect_x_list) - min(intersect_x_list))/x_tick_num,2)
        
        y_extent = max(v_proj_list) - min(v_proj_list)
        x_extent = max(intersect_x_list) - min(intersect_x_list)
        ...
        xlim_pos = max(intersect_x_list) + x_tick_spacing
        xlim_neg = min(intersect_x_list) - x_tick_spacing
